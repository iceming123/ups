...
func TestOutGenesisJson(t *testing.T) {
	...
	b, _ := ioutil.ReadFile("E:\\upschain\\src\\github.com\\upschain\\ups\\cmd\\gups\\genesis.json")
	...
	for i := 1; i <= 52; i++ {
		...
		e := ioutil.WriteFile("E:\\upschain\\src\\github.com\\upschain\\ups\\cmd\\gups\\"+fmt.Sprintf("genesis_%s.json", strconv.Itoa(i)), bytes, os.ModePerm)
		if e != nil {
